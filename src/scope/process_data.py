...
def assign_unassigned_mutations_to_phylogeny(
    ...
    mutation_groups_X = pd.merge(mutation_group_labels, X, left_on="mutation_group", right_index=True, how="left")
    mutations_not_selected = mutation_groups_X[mutation_groups_X[0] < 0.5]['mutation'].to_list()
        
    F_not = cell_fractions_F.loc[:, mutations_not_selected]
    F_bar_selected = F_bar.loc[:, mutation_groups_selected]

    df1 = F_not
    df2 = F_bar_selected

    df1_values = df1.values.T
    df2_values = df2.values.T

    dist = cdist(df1_values, df2_values, metric='euclidean')
    logits = -dist / tau
    responsibilities = np.exp(logits) / np.exp(logits).sum(axis=1, keepdims=True)

    max_resp = responsibilities.max(axis=1)
    best_idx = np.argmax(responsibilities, axis=1)
    best_clusters = F_bar_selected.columns[best_idx]

    assigned = [
        int(best_clusters[i]) if max_resp[i] >= cutoff else np.nan
        for i in range(len(best_clusters))
    ]

    update_df = pd.DataFrame({
        "mutation": mutations_not_selected,
        "reassigned_mutation_group": assigned
    })

    merged = mutation_groups_X.merge(update_df, on="mutation", how="left")
    merged["mutation_group_orig"] = merged["mutation_group"].copy()
    merged["mutation_group"] = merged["reassigned_mutation_group"].combine_first(merged["mutation_group"])
    merged.rename(columns={0: 'selected'}, inplace=True)
    merged["selected"] = merged["selected"].fillna(0).astype(int)
    merged["selected_orig"] = merged["selected"].copy()
    merged.loc[~merged["reassigned_mutation_group"].isna(), "selected"] = 1
    merged["mutation_group"] = merged["mutation_group"].astype(int)
